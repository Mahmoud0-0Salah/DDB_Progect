...
func (s *SlaveService) switchDatabase(dbName string) error {
	if dbName == "" {
		return fmt.Errorf("database name cannot be empty")
	}
	_, err := s.db.Exec(fmt.Sprintf("USE %s", dbName))
	if err != nil {
...
func (s *SlaveService) checkRecordExists(dbName, table, id string) (bool, error) {
	if err := s.switchDatabase(dbName); err != nil {
		return false, err
	}
	var count int
...
func (s *SlaveService) handleReplicateInsert(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.switchDatabase(dbName); err != nil {
		log.Printf("Failed to switch to database %s on slave: %v", dbName, err)
	...
	if _, err := s.db.Exec(query); err != nil {
		...
		http.Error(w, "Insert failed on slave: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func (s *SlaveService) handleReplicateCreateTable(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := s.db.Exec(query); err != nil {
		...
		http.Error(w, "Create table failed on slave: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func (s *SlaveService) handleReplicateUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := s.db.Exec(query); err != nil {
		...
		http.Error(w, "Update failed on slave: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func (s *SlaveService) handleReplicateDelete(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := s.db.Exec(query); err != nil {
		...
		http.Error(w, "Delete failed on slave: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func (s *SlaveService) handleSelect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		http.Error(w, "Failed to select records: "+err.Error(), http.StatusInternalServerError)
		return
	}
	defer rows.Close()
